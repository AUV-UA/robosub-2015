...
//    VideoCapture camera = new VideoCapture("/home/auvua/Videos/2015-07-24-down.mp4");
    ...
    int[] initValues = { 0,  53,  15,  93,  58,  255 };
    ...
    while(true) {
      ...
      for (MatOfPoint contour : contours) {
        ...
        double aspectRatio = Math.max(1.0 * box.size.height / box.size.width, 1.0 * box.size.width / box.size.height);

        //        System.out.println(rectangularity);

        if(area > 2000 && rectangularity > maxRectangularity && aspectRatio > 3.0) {
          System.out.println(String.format("Area: %-6f Rect: %-1.4f", area, rectangularity));
